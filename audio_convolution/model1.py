...
class Variational2DAutoEncoder(nn.Module):
    def __init__(self, input_dimensions, hidden_dimensions=200, latent_space_dimensions=20):
        ...
        # Encoder
        self.convolutional_layer1 = nn.Conv2d(in_channels=1, out_channels=32, kernel_size=3, stride=1, padding=1)
        self.convolutional_layer2 = nn.Conv2d(in_channels=32, out_channels=64, kernel_size=3, stride=1, padding=1)
        self.hidden_layer_encoder = nn.Linear(in_features=64 * (input_dimensions // 4) * (input_dimensions // 4), out_features=hidden_dimensions)
        ...
        self.reconstruction_layer1 = nn.Linear(in_features=hidden_dimensions, out_features=64 * (input_dimensions // 4) * (input_dimensions // 4))
        self.deconvolutional_layer1 = nn.ConvTranspose2d(in_channels=64, out_channels=32, kernel_size=3, stride=1, padding=1)
        self.deconvolutional_layer2 = nn.ConvTranspose2d(in_channels=32, out_channels=1, kernel_size=3, stride=1, padding=1)

        self.relu_activation = nn.ReLU()

    def encode(self, x):
        # Assuming input is a 2D matrix of size (420, 420)
        x = x.view(-1, 1, 420, 420)
    ...
    def decode(self, z):
        ...
        x = x.view(x.size(0), 64, (input_dimensions // 4), (input_dimensions // 4))
